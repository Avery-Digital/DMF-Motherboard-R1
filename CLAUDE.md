...
- **TIM6 reserved for CMD_MEASURE_ADC.** TIM6 (basic timer on APB1) is used in one-pulse mode for deterministic timing in `Command_ExecuteMeasureADC()`. PSC=2399 gives 10 µs ticks at 240 MHz APB1 timer clock. Timer starts BEFORE any commands are sent; value = PWM_PHASE_SYNC_TIME_US + (num_switches × SWITCH_ENABLE_TIME_US) + (user_delay_ms × 1000). Sequence within timer window: PWMPhaseSync (0x0A81, resets driver board TIM2/TIM1/TIM8 counters) → SET_LIST_OF_SW. Clock is enabled/disabled per-use. Do not repurpose TIM6 without updating this command.
