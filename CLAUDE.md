...
- **TIM6 reserved for CMD_MEASURE_ADC.** TIM6 (basic timer on APB1) is used in one-pulse mode for deterministic timing in `Command_ExecuteMeasureADC()`. PSC=239 gives 1 µs ticks at 240 MHz APB1 timer clock (max 65.5 ms). Phase 3 sends PWMPhaseSync (0x0A81) and waits for response (confirms driver board TIM2/TIM1/TIM8 counters reset). TIM6 starts AFTER this confirmation; value = (num_switches × SWITCH_ENABLE_TIME_US) + (user_delay_ms × 1000). SET_LIST_OF_SW fires within the timer window. Clock is enabled/disabled per-use. Do not repurpose TIM6 without updating this command.
